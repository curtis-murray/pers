...
#'
#' @import dplyr
model_emotions_tbl <- function(tbl, input, batch_size = NULL, quiet = T, show_prior = F, show_positivity = F){
  ...
  #stopifnot("Input error: tbl has 0 rows" = nrow(tbl) < 1)
  ...
  while((tbl_itr-1)*batch_size < tbl_len){
    ...
    out <- bind_rows(out, tbl[((tbl_itr-1)*batch_size+1):min(tbl_len,tbl_itr*batch_size),] %>%
                       ...
                       left_join(model, by = "word") %>%
                       drop_na() %>%
                       group_by(Report_tmp, Emotion,prior, group_id) %>%
                       summarise(ls = sum(log(likelihood)),
